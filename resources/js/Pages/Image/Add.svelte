<script>
    ...
    import { apiAddTag } from "@/js/wayfinder/actions/App/Http/Controllers/TagController";
    ...
    let filteredTags = $derived(
        startsWith
            ? tags.filter((t) =>
                  t.tag_name.toUpperCase().startsWith(startsWith),
              )
            : tags,
    );
...
<h1 class="mb-4 text-lg font-bold">{$i18n.t("translate.addnewimage")}</h1>
...
<form onsubmit={submit} class="space-y-4">
        ...
        {#if form.progress}
            ...
            <div class="inline-flex items-center gap-1">
                ...
                {#if form.processing && form.progress?.percentage < 100}
                    <span class="text-sm"
                        >{$i18n.t("translate.uploadingimage")}</span
                    >
                {/if}
                {#if form.processing && form.progress?.percentage === 100}
                    <span class="text-sm"
                        >{$i18n.t("translate.processingimage")}</span
                    >
                {/if}
            </div>
        {/if}
    ...
    <div class="bg-base-300 rounded-base collapse mt-4">
    ...
    <div>
        ...
        {:else}
            <div class="mt-4">
                <div
                    role="alert"
                    class="alert alert-error alert-soft inline-flex"
                >
                    <CircleAlert class="h-6 w-6 shrink-0" />
                    <div>
                        <h3 class="font-bold">
